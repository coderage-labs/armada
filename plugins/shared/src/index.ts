/**
 * armada-shared — Shared utilities for fleet plugins.
 *
...
export const FLEET_PENDING_SYM = 'armada-session-pending';
export const FLEET_COORD_CB_SYM = 'armada-coordinator-callbacks';
export const FLEET_OUTBOUND_SYM = 'armada-outbound-tasks';
export const FLEET_INBOUND_SYM = 'armada-inbound-contexts';
...
function getDefaultDownloadDir(): string {
  ...
  return `${home}/.openclaw/workspace/fleet-downloads`;
}

/** Decode base64 attachments to local files, return descriptive text + file paths */
export function decodeAttachments(attachments: any[], logger?: FleetLogger, downloadDir?: string): string {
...
export function decodeAttachmentsAsMarkers(attachments: any[], logger?: FleetLogger, downloadDir?: string): string {
  if (!attachments?.length) return '';
  const dlDir = downloadDir || getDefaultDownloadDir();
  mkdirSync(dlDir, { recursive: true });
 ...
 */
export function extractFileMarkers(text: string, logger?: FleetLogger): {
  cleanedText: string;
...
/**
 * Core fleet task dispatch — extracts file markers, encodes attachments,
 * POSTs to the target's /fleet/task endpoint.
...
export async function dispatchArmadaTask(
  ...
  try {
    const resp = await fetch(`${opts.targetUrl}/fleet/task`, {
      method: 'POST',
      headers: {
...
export interface OutboundTask {
  taskId: string;
  /** Session that called fleet_task */
  sessionKey: string;
  target: string;
}
...
/**
 * Set up a fleet session for injecting agent turns.
...
export async function createInboundContext(
  ...
  const sessionKey = opts.sessionKey || (
    opts.targetAgent
      ? `agent:${opts.targetAgent}:fleet:${opts.from}:${short}`
      : `fleet:${opts.from}:${short}`
  );
  const chatId = opts.targetAgent ? `fleet:${opts.targetAgent}:${opts.from}` : `fleet:${opts.from}`;
  ...
  await recordInboundSession({
    ...
    sessionKey,
    ctx,
    updateLastRoute: { sessionKey, channel: 'fleet', to: chatId, accountId: 'fleet' },
...
export function resetInboundIdleTimer(inbound: InboundContext, logger: FleetLogger = console): void {
...
export function resetPingWatchdog(inbound: InboundContext, logger: FleetLogger = console): void {
...
export function resetProgressTimer(inbound: InboundContext, logger: FleetLogger = console): void {
...
/**
 * Fire a single agent turn in an inbound context's fleet session.
 ...
 */
export function maybeFinalize(inbound: InboundContext, turnComplete: boolean, logger: FleetLogger = console): boolean {
 ...
 * - Coordinator loop: if agent calls fleet_task, waits for sub-task results
...
export async function injectAndWaitForResponse(
  ...
  const short = opts.taskId.split('-').slice(0, 2).join('-');
  const sessionKey = opts.sessionKey || `fleet:${opts.from}:${short}`;
  const chatId = `fleet:${opts.from}`;
  ...
  try {
    ...
    // If the agent called fleet_task during its turn and responded without
    ...
    while (pendingCount > 0 && !timedOut) {
      ...
      const resultMessage = [
        ...
        `\n[INSTRUCTIONS: Before returning this result or delegating the next task, VERIFY the deliverable matches the original request. Use \`image\` to check images, \`Read\` to check files. If the result is wrong (e.g. wrong image, broken output), re-delegate with clearer instructions. If verified or not applicable, continue — use fleet_task for the next step, or provide your final answer with TASK_COMPLETE if all steps are complete.]`,
      ].join('\n');

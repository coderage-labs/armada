 ...
 * isolated sessions (fleet:target:taskShort) via the shared injectAndWaitForResponse
...
const sessionPendingTasks = getOrCreateGlobalMap<string, Set<string>>(FLEET_PENDING_SYM);
const coordinatorCallbacks = getOrCreateGlobalMap<string, (result: SubTaskResult) => void>(FLEET_COORD_CB_SYM);

// ── State ───────────────────────────────────────────────────────────

let _config: FleetControlConfig | null = null;
let _api: any = null;
...
async function fetchAgents(): Promise<FleetAgent[]> {
  if (!_config?.fleetApiUrl) return [];
  try {
    const resp = await fetch(`${_config.fleetApiUrl}/api/agents`, {
      headers: { 'Authorization': `Bearer ${_config.fleetApiToken}` },
      signal: AbortSignal.timeout(10_000),
    });
    if (!resp.ok) return [];
    return await resp.json() as FleetAgent[];
  } catch { return []; }
}

function agentToUrl(agent: FleetAgent): string {
  return `http://fleet-${agent.name}:18789`;
}

async function fleetApiGet(path: string, format: 'json' | 'text' = 'json'): Promise<any> {
  if (!_config?.fleetApiUrl) return { error: 'Fleet API URL not configured' };
  try {
    const resp = await fetch(`${_config.fleetApiUrl}${path}`, {
      headers: {
...
async function fleetApiPost(path: string, body?: any): Promise<any> {
  if (!_config?.fleetApiUrl) return { error: 'Fleet API URL not configured' };
  try {
    const resp = await fetch(`${_config.fleetApiUrl}${path}`, {
      method: 'POST',
...
async function fleetApiDelete(path: string): Promise<any> {
  if (!_config?.fleetApiUrl) return { error: 'Fleet API URL not configured' };
  try {
    const resp = await fetch(`${_config.fleetApiUrl}${path}`, {
      method: 'DELETE',
...
async function fleetApiPut(path: string, body?: any): Promise<any> {
  if (!_config?.fleetApiUrl) return { error: 'Fleet API URL not configured' };
  try {
    const resp = await fetch(`${_config.fleetApiUrl}${path}`, {
      method: 'PUT',
...
/**
 * Fetch tool definitions from the fleet API and register them as OpenClaw tools.
 * Each API endpoint that calls registerToolDef() becomes an auto-generated tool.
 */
async function loadApiTools(): Promise<void> {
  const defs = await fleetApiGet('/api/meta/tools') as ToolDef[];
  if (!Array.isArray(defs) || defs.length === 0) {
    _logger.warn('[armada-control] No tool definitions found from API');
    return;
  }

  // Skip fleet_task, fleet_status, fleet_contacts — registered manually with special logic
  const MANUAL_TOOLS = new Set(['fleet_task', 'fleet_status', 'fleet_contacts']);
...
async function executeApiTool(def: ToolDef, args: Record<string, any>): Promise<any> {
  ...
  if (def.method === 'GET') {
    ...
    return fleetApiGet(resolvedPath, def.responseFormat === 'text' ? 'text' : 'json');
  }

  if (def.method === 'DELETE') {
    return fleetApiDelete(resolvedPath);
  }
  ...
  if (def.method === 'PUT') {
    return fleetApiPut(resolvedPath, Object.keys(body).length > 0 ? body : undefined);
  }

  return fleetApiPost(resolvedPath, Object.keys(body).length > 0 ? body : undefined);
}
...
/**
 * Inject a fleet result into an isolated coordinator session.
 *
...
async function injectIntoCoordinatorSession(taskId: string, pending: PendingTask, resultBody: string, from: string, attachmentPaths: string[] = []): Promise<void> {
  ...
  if (deliveryParts.length === 0) {
    ...
    await callGatewayRpc(_api, 'agent', {
      ...
      message: [
        `[FLEET RESULT] "${from}" completed a task.`,
        `Task ID: ${taskId}`,
        ...contextLines,
        `Result:\n${resultBody}`,
...
export default function register(api: any) {
  ...
  _logger.info(`[armada-control] Operator plugin loaded — fleet API: ${_config.fleetApiUrl}`);

  // ── Tool: fleet_task (factory-style for session key capture) ────
  ...
  (api.registerTool as any)((ctx: any) => {
    ...
    return {
      ...
      execute: async (_id: string, args: { target: string; message: string; project?: string }, context: any) => {
        ...
        const coordinatorSessionKey = `fleet:${args.target.toLowerCase()}:${taskShort}`;
        ...
        const result = await dispatchArmadaTask({
          ...
          fromRole: 'operator',
          message: args.message,
          callbackUrl: _config!.callbackUrl ? `${_config!.callbackUrl}/fleet/result` : '',
  ...
  api.registerTool({
    name: 'fleet_status',
    description: 'Check the status of all Armada instances. Shows which agents are running and available for fleet_task.',
  ...
  api.registerTool({
    name: 'fleet_contacts',
    description: 'List all running Armada instances you can send tasks to via fleet_task.',
    parameters: { type: 'object', properties: {} },
    execute: async () => {
      const agents = await fetchAgents();
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { target: string; taskId?: string; message: string }) => {
      ...
      const url = agentToUrl(agent);
      try {
        const res = await fetch(`${url}/fleet/steer`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { fromAgent: string; path: string; toAgent: string; destPath?: string }) => {
      ...
      try {
        const res = await fetch(`${_config!.fleetApiUrl}/api/files/transfer`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${_config!.fleetApiToken || ''}`,
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { action: string; projectId?: string; issueNumber?: number }) => {
      ...
      try {
        const res = await fetch(`${_config!.fleetApiUrl}${endpoint}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${_config!.fleetApiToken || ''}`,
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { workflowId: string; triggerRef?: string; vars?: Record<string, any> }) => {
      ...
      try {
        const res = await fetch(`${_config!.fleetApiUrl}/api/workflows/${args.workflowId}/run`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${_config!.fleetApiToken || ''}`,
  ...
  // Fetch tool definitions from fleet API and register them dynamically.
  ...
  if (typeof api.registerHttpRoute === 'function') {
    api.registerHttpRoute({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        // Decode attachments to workspace fleet-downloads dir (allowed by message tool)
        const attachmentPaths: string[] = [];
        if (body.attachments?.length) {
          const { mkdirSync, writeFileSync } = require('node:fs');
          const workspaceDir = process.env.WORKSPACE_DIR || `${process.env.HOME || '/home/node'}/.openclaw/workspace`;
          const dlDir = `${workspaceDir}/fleet-downloads`;
          ...
          if (cleaned.channel && cleaned.to) {
            try {
              ...
              await callGatewayRpc(_api, 'send', {
                ...
                idempotencyKey: `fleet-result-fallback-${taskId}`,
              });
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        if (event === 'workflow.gate') {
          const preview = previousOutput && previousOutput.length > 500
            ...
            : previousOutput || '';
          message = [
            `[FLEET NOTIFICATION] Workflow gate reached.`,
            `⏸️ Workflow "${workflowName || 'unknown'}" paused at gate "${stepId || 'unknown'}"`,
            `Run ID: ${runId}`,
            preview ? `\nPrevious step output:\n${preview}` : '',
            `\nReview the output and approve with: fleet_workflow_run_approve(runId="${runId}", stepId="${stepId}")`,
            `Or ask Chris if you're unsure.`,
          ].filter(Boolean).join('\n');
        } else if (event === 'workflow.completed') {
          message = `[FLEET NOTIFICATION] ✅ Workflow "${workflowName || 'unknown'}" completed. Run ID: ${runId}`;
        } else if (event === 'workflow.failed') {
          message = `[FLEET NOTIFICATION] ❌ Workflow "${workflowName || 'unknown'}" failed. Run ID: ${runId}. Check the run for details.`;
        } else {
          message = `[FLEET NOTIFICATION] Event: ${event}. ${JSON.stringify(body)}`;
        }
        ...
        try {
          await callGatewayRpc(_api, 'agent', {
            ...
            deliver: true,
            bestEffortDeliver: true,
            idempotencyKey: `fleet-notify-${runId}-${stepId || event}`,
          }, 30_000);
  ...
  api.registerService({
    id: 'armada-control',
    start: async () => {
      _logger.info(`[armada-control] Started — fleet API: ${_config!.fleetApiUrl}`);
    },
    stop: async () => {

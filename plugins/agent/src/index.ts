/**
 * armada-agent — Fleet communication plugin for managed OpenClaw instances.
 *
...
interface FleetConfig {
  ...
  /** Node agent proxy URL — preferred over fleetApiUrl for remote node deployments */
  proxyUrl?: string;
  projects?: string[];
}

/**
 * Get the base URL for fleet API calls.
 * Prefers proxyUrl (node agent gateway proxy) over fleetApiUrl (direct control plane).
 * This allows remote node deployments where the control plane isn't directly reachable.
 */
function getApiBaseUrl(): string {
  return _config?.proxyUrl || _config?.fleetApiUrl || '';
}
...
function pushUsage(context: any): void {
  if (!_config?.fleetApiToken) return;
...
const outboundTasks = getOrCreateGlobalMap<string, OutboundTask>(FLEET_OUTBOUND_SYM);
const inboundContexts = getOrCreateGlobalMap<string, InboundContext>(FLEET_INBOUND_SYM);

let _config: FleetConfig | null = null;
let _draining = false;
let _activeTasks = 0;
let _api: any = null;
let _logger: FleetLogger = console;
...
const NATIVE_TOOLS = new Set(['fleet_task', 'fleet_contacts', 'fleet_share', 'fleet_steer', 'fleet_status', 'fleet_workflow_context', 'fleet_request_rework']);

interface FleetToolDef {
  name: string;
  description: string;
  method: string;
  path: string;
 ...
 * When calling the control plane directly (fleetApiUrl only), fleetApiToken is used.
 */
function getToolAuthHeaders(): Record<string, string> {
  if (!_config) return {};
  const token = _config.fleetApiToken || _config.hooksToken;
  return token ? { 'Authorization': `Bearer ${token}` } : {};
}

async function loadFleetTools(api: any) {
  // Require a base URL; fleetApiToken is optional when routing via proxyUrl (relay handles auth)
  if (!_config || !getApiBaseUrl()) return;
  if (!_config.proxyUrl && !_config.fleetApiToken) return; // direct: token required
  ...
  try {
    ...
    if (!resp.ok) {
      _logger.warn(`[armada-agent] Failed to fetch fleet tools: ${resp.status}`);
      return;
    }

    const tools = (await resp.json()) as FleetToolDef[];
    let registered = 0;
    ...
    _logger.info(`[armada-agent] Loaded ${registered} fleet tools from control plane`);
  } catch (err: any) {
    _logger.warn(`[armada-agent] Error loading fleet tools: ${err.message}`);
  }
}

async function executeFleetTool(def: FleetToolDef, args: Record<string, any>): Promise<any> {
  // Require a base URL; fleetApiToken is optional when routing via proxyUrl (relay handles auth)
  if (!_config || !getApiBaseUrl()) {
    return { error: 'Armada API not configured' };
  }
  if (!_config.proxyUrl && !_config.fleetApiToken) {
    return { error: 'Armada API authentication not configured (set fleetApiToken or proxyUrl)' };
  }
...
export default function register(api: any) {
  ...
  _config = {
    ...
    hooksToken: pluginConfig.hooksToken || pluginConfig.fleetApiToken || '',
  ...
  api.registerTool({
    name: 'fleet_task',
    description: 'Send an async task to a fleet agent. Results are delivered automatically when complete. Use this to delegate work to other agents in your fleet.',
    parameters: {
      type: 'object',
      properties: {
    ...
    execute: async (_id: string, args: { target: string; message: string; project?: string }, context: any) => {
      ...
      if (!sessionKey) {
        ...
        if (inboundContexts.size === 1) {
          ...
          _logger.warn(`[armada-agent] fleet_task called without context.sessionKey or ALS, and ${inboundContexts.size} active inbound contexts — picking first`);
  ...
  api.registerTool({
    name: 'fleet_contacts',
    description: 'List available fleet agents — the agents you can communicate with.',
    ...
    execute: async () => {
      ...
      try {
        ...
        if (!res.ok) {
          return { error: `Failed to fetch agents: ${res.status}` };
        }

        const agents = await res.json() as Array<{ name: string; role?: string }>;
        return {
          contacts: agents.map(a => ({ name: a.name, role: a.role || 'unknown' })),
        };
      } catch (err: any) {
        return { error: `Failed to fetch agents: ${err.message}` };
      }
    },
  });
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { path: string }) => {
      ...
      if (!_config || !getApiBaseUrl()) return { error: 'Armada API URL not configured' };

      try {
        const res = await fetch(`${getApiBaseUrl()}/api/files/share`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(_config.fleetApiToken ? { 'Authorization': `Bearer ${_config.fleetApiToken}` } : {}),
        ...
        if (!res.ok) return { error: data.error || `Share failed (${res.status})` };
        return {
          shared: true,
          ref: data.ref,
  ...
  api.registerTool({
    ...
    execute: async (_id: string, args: { taskId: string; message: string }) => {
      if (!args?.taskId || !args?.message) return { error: 'Missing taskId or message' };
      if (!getApiBaseUrl() || !_config?.hooksToken) {
        return { error: 'Armada API not configured' };
      }

      try {
        const res = await fetch(`${getApiBaseUrl()}/api/tasks/${encodeURIComponent(args.taskId)}/steer`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${_config!.hooksToken}`,
        ...
        if (!res.ok) {
          const errorText = await res.text().catch(() => 'unknown error');
          return { error: `Steer failed (${res.status}): ${errorText}` };
        }

        return { status: 'steered', taskId: args.taskId };
      } catch (err: any) {
        return { error: `Failed to steer task: ${err.message}` };
      }
    },
  });
  ...
  api.registerTool({
    ...
    execute: async () => ({
      ...
      fleetApiUrl: getApiBaseUrl(),
    }),
  });

  // ── Tool: fleet_workflow_context ───────────────────────────────

  api.registerTool({
    name: 'fleet_workflow_context',
    description: 'Get the full context of the workflow you are currently executing in. Shows all steps, who did what, their roles, outputs, and any rework history. Use this to understand what other agents have done and make informed decisions.',
    parameters: {
      type: 'object',
      properties: {
        runId: { type: 'string', description: 'Workflow run ID (from your task context)' },
      },
      required: ['runId'],
    },
    execute: async (_id: string, args: { runId: string }) => {
      ...
      try {
        const res = await fetch(`${getApiBaseUrl()}/api/workflows/runs/${args.runId}/context`, {
          headers: { 'Authorization': `Bearer ${_config!.hooksToken}` },
          signal: AbortSignal.timeout(10_000),
        });
        if (!res.ok) {
          const errText = await res.text().catch(() => 'unknown');
          return { error: `Failed to get workflow context: ${res.status} ${errText}` };
        }
        return await res.json();
      } catch (err: any) {
        return { error: `Failed to reach control plane: ${err.message}` };
      }
    },
  });

  // ── Tool: fleet_request_rework ─────────────────────────────────

  api.registerTool({
    name: 'fleet_request_rework',
    description: 'Request that a previous workflow step be re-done with specific feedback. Use when you discover that a previous step\'s output is insufficient, incorrect, or missing something you need. Your current step will pause until the rework completes, then you will be re-dispatched with the updated context.',
    parameters: {
      type: 'object',
      properties: {
        runId: { type: 'string', description: 'Workflow run ID' },
        targetStepId: { type: 'string', description: 'The step ID to send back for rework' },
        feedback: { type: 'string', description: 'Detailed feedback explaining what needs to change and why' },
      },
      required: ['runId', 'targetStepId', 'feedback'],
    },
    execute: async (_id: string, args: { runId: string; targetStepId: string; feedback: string }, context: any) => {
      if (!getApiBaseUrl() || !_config?.hooksToken) {
        return { error: 'Armada API not configured' };
      }

      // Determine the requesting step ID from the current task context
      // The task ID format for workflow tasks is: wf-<runId8>-<stepId>
      let requestingStepId: string | undefined;
      const sessionKey = context?.sessionKey || turnSessionStore.getStore();
      if (sessionKey) {
        const inbound = inboundContexts.get(sessionKey);
        if (inbound?.taskId) {
          // Extract step ID from task ID: wf-XXXXXXXX-<stepId>
          const parts = inbound.taskId.split('-');
          if (parts.length >= 3 && parts[0] === 'wf') {
            requestingStepId = parts.slice(2).join('-');
          }
        }
      }

      if (!requestingStepId) {
        return { error: 'Could not determine current step ID. This tool can only be used during workflow execution.' };
      }

      try {
        const res = await fetch(`${getApiBaseUrl()}/api/workflows/runs/${args.runId}/rework`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${_config!.hooksToken}`,
        ...
        if (!res.ok) {
          const errText = await res.text().catch(() => 'unknown');
          return { error: `Rework request failed: ${res.status} ${errText}` };
        }
        ...
        return {
          ...
          targetStepId: args.targetStepId,
          message: `Rework requested on step "${args.targetStepId}". Your step will pause and resume once the rework completes.`,
          ...result,
        };
      } catch (err: any) {
        return { error: `Failed to reach control plane: ${err.message}` };
  ...
  // the relay — fleetApiToken is not required (proxy handles control-plane auth).
  // For direct deployments (fleetApiUrl only), fleetApiToken is required.

  if (getApiBaseUrl() && (_config.proxyUrl || _config.fleetApiToken)) {
    loadFleetTools(api).catch(err => {
      _logger.warn(`[armada-agent] Failed to load fleet tools: ${err.message}`);
    });
  }
  ...
  // ── HTTP: Notify — receive notifications from fleet control ────────
  ...
  api.registerHttpRoute({
    ...
    handler: async (_req: IncomingMessage, res: ServerResponse) => {
      try {
        ...
        // Transform gateway response to fleet format
  ...
  api.registerHttpRoute({
    ...
    handler: async (req: IncomingMessage, res: ServerResponse) => {
      try {
        ...
        // sessions.list returns canonical keys (e.g. "agent:forge:fleet:operator:nudge-123")
        // but the session store may use a shorter key (e.g. "fleet:operator:nudge-123").
        ...
        // Normalize content blocks for the fleet UI
  ...
  async function sendHeartbeats() {
    if (!getApiBaseUrl() || !_config?.fleetApiToken) return;

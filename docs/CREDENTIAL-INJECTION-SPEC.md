...
[credential]
    helper = /usr/local/bin/fleet-credential-helper
```

The helper is a small script/binary that reads a credentials file maintained by the node agent:

```
# /etc/fleet/git-credentials.json (written by node agent, read-only to agent process)
{
  "credentials": [
    {
      "host": "github.com",
...
```bash
#!/bin/sh
# /usr/local/bin/fleet-credential-helper
# Reads /etc/fleet/git-credentials.json
...
```
git push → credential helper → Unix socket → node agent → fleet DB → token
```
...
### Fleet API proxy endpoints

Fleet control already has the integration credentials in its DB. Add proxy endpoints:
...
### Agent-side: `fleet-issue` tool

Rather than agents constructing HTTP requests to the proxy, register fleet tools that abstract it:

```
fleet_issue_list       - List issues for a project
fleet_issue_get        - Get issue details
fleet_issue_comment    - Add a comment
fleet_issue_transition - Change issue status
fleet_issue_search     - Search issues (JQL, GitHub query, etc.)
fleet_issue_sync       - Trigger issue sync for a project
```
...
This works naturally — nexus is a member of the project, so it has proxy access. It uses `fleet_issue_list` and `fleet_issue_search` to read the backlog, `fleet_issue_comment` to annotate, `fleet_issue_transition` to update status. No tokens anywhere in its context.
...
1. Fleet control calls node agent: `POST /agents/:name/credentials/sync`
...
3. Fleet control computes: all projects the agent is a member of → all integrations with VCS capability → all repo URLs + credentials
...
1. Fleet control calls credential sync
...
Separate from credential injection but related — periodic sync of external issues into fleet's `external_issues` table.

### Sync triggers

- **Manual**: "Sync Now" button in UI, or `fleet_project_integration_sync` tool
...
2. PM agent uses `fleet_issue_get` to read full details (via proxy, no tokens)
3. PM agent triages: priority, assignment, workflow trigger
4. PM agent uses `fleet_issue_comment` to annotate the external issue
...
1. Mounts a credentials volume: `/etc/fleet/` (read-only to agent process)
...
- `/usr/local/bin/fleet-credential-helper` binary
- Default `~/.gitconfig` with credential helper configured
- Correct file permissions on `/etc/fleet/`
...
```
fleet_issue_list       - List issues for a project backlog
fleet_issue_get        - Get full issue details by key
fleet_issue_comment    - Add comment to an issue
fleet_issue_transition - Change issue status/state
fleet_issue_search     - Search issues with provider-specific query
```
...
```
# Via fleet tools (registered by Armada plugin)
fleet_issue_list(project="acme", filters={status: "open"})
fleet_issue_get(project="acme", issueKey="ACME-123")
fleet_issue_comment(project="acme", issueKey="ACME-123", comment="PR opened: #456")
fleet_issue_transition(project="acme", issueKey="ACME-123", status="In Review")
```
...
3. Fleet plugin calls proxy endpoints
...
2. **Agent image**: Inject at container creation (bind mount + docker exec). No upstream image changes required, fleet-specific, works with any OpenClaw image.

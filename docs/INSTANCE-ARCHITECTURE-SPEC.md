...
Armada currently deploys **one Docker container per agent**. Each container is a full OpenClaw instance — Node.js runtime, gateway, plugin system, workspace — consuming ~2 GB of memory. A fleet of 10 agents eats 20 GB before any work happens.
...
| Term | Definition |
|------|-----------|
| **Node** | A Docker host running the fleet node-agent. Manages containers. |
...
| **Lead agent** | The primary agent in an instance (agent id `main`). Receives fleet tasks, coordinates internal agents. Every instance has exactly one lead. |
...
CREATE TABLE instances (
  ...
  url           TEXT NOT NULL,               -- e.g. "http://fleet-dev-team:18789"
...
```
             ...
             └─────────────┬──────────────┘
                           │
             ┌─────────────▼──────────────┐
             │      RUNNING         ◄──────┼──── fleet_start_instance()
             │  - Healthy, accepting       │
             │    agent placements         │
             └──┬──────────┬──────────┬───┘
                │          │          │
    fleet_drain │  fleet_  │   fleet_ │
...
4. **Create volume** — directory structure for instance data:
   ```
   /data/fleet/volumes/{instance-name}/
     .openclaw/
...
| Endpoint | Purpose |
|----------|---------|
| `GET /hooks/fleet/health` | Basic liveness — `{ healthy, uptime, version }` |
| `GET /hooks/fleet/status` | Detailed — `{ status, activeAgents, activeSessions, memoryUsage }` |
| `GET /hooks/fleet/capacity` | Placement info — `{ currentAgents, maxAgents, availableMemory }` |
...
```
User: fleet_spawn(templateId: "forge", name: "forge")
                    │
                    ▼
          ┌─────────────────┐
          │  Resolve template│
...
| Armada name | Unique across fleet | `forge` (same as today) |
...
| Skills | Re-installed on target | Using fleet skill install flow |
...
- The `fleet_task()` tool is available to all agents but uses the calling agent's identity
...
```
/data/fleet/volumes/{instance-name}/.openclaw/agents/{agent-id}/workspace/
  .git-credentials          ← per-agent git credentials
  .credential-helper.json   ← credential helper config
```

The existing fleet credential helper reads from workspace-level files, making this transparent to the agent. Each agent's credential-helper config points to its own workspace path, ensuring isolation without needing per-agent environment variables.
...
```
fleet_task("forge", "implement feature X")

1. Fleet-control looks up agent "forge" → instance "dev-team" at http://fleet-dev-team:18789
2. POST http://fleet-dev-team:18789/hooks/fleet/task
...
| File | Change |
|------|--------|
| Tool definitions | Add instance management tools: `fleet_spawn_instance`, `fleet_instances`, `fleet_instance_capacity`. Modify `fleet_spawn` to accept `instanceId`. Add `fleet_transfer`. |
...
```
fleet_spawn_instance(templateId, name, nodeId?)
  → Create a new instance from an instance template

fleet_instances()
  → List all instances with status and capacity

fleet_instance_capacity(name)
  → Get detailed capacity info for an instance

fleet_spawn(templateId, name, instanceId?)
  → Spawn agent into a specific instance (or auto-place)

fleet_transfer(agentName, targetInstanceId?)
  → Move agent to another instance

fleet_instance_stop(name)
fleet_instance_start(name)
fleet_instance_destroy(name)
fleet_instance_logs(name, tail?)
```
...
**Changes:**
1. Fleet-agent plugin gains agent CRUD endpoints (`/hooks/fleet/agents`)
2. `fleet_spawn` can target an existing instance
...
**Result:** Fleet can dynamically manage agent placement across nodes and instances.
...
Fleet-control generates this config. Each agent in the list gets:
...
For fleet tasks, the armada-agent plugin uses `callGateway('agent', { agentId: 'forge' })` to inject a task into a specific agent's session.

### 10.3 Native Inter-Agent Communication

Agents within the same instance can communicate via OpenClaw's native subagent system (`sessions_spawn`). This is zero-overhead — no HTTP, no serialisation, no network. Fleet should prefer co-locating collaborating agents to take advantage of this.
...
Fleet spawn flow: armada-agent plugin writes updated `openclaw.json` → sends SIGUSR1 → waits for instance to become healthy → confirms agent is available. For instances with many active agents, fleet should send a drain notification before the SIGUSR1 to minimise disruption (see §11.1, point 5).
...
| # | Question | Answer |
|---|----------|--------|
| 1 | **Internal Agent API** — Is there an HTTP CRUD API for managing agents, or must we edit config + restart? | **Config-file based only.** `agents.list[]` in `openclaw.json` defines agents. No HTTP CRUD API exists. Fleet must: read current config → merge new agent entry → write config → send SIGUSR1. |
| 2 | **Hot-add agents** — Can `agents.list[]` be modified without full restart? | **No.** Requires config change + SIGUSR1 restart. SIGUSR1 is graceful — drains active turns (~30s max). Fleet spawn flow: update `openclaw.json` → SIGUSR1 → wait for healthy → confirm. Acceptable for spawn operations. |
...
| 10 | **Credential scoping** | Env vars are global (`env.vars`), so **credential scoping must use file-based helpers**. Each agent's workspace gets its own `.git-credentials` or credential-helper config. The fleet credential helper already supports per-workspace credential injection. |
| 11 | **Plugin isolation** | **No — plugins are instance-global.** All agents share the same loaded plugins. This is acceptable for fleet: the armada-agent plugin is the same for all agents. Role-based tool filtering is handled by the plugin at runtime, not at the plugin loading level. |

### 11.1 Design Implications

These answers shape the implementation in several important ways:

1. **Fleet spawn ≠ Docker create.** Spawning an agent within an instance means: read `openclaw.json` → inject agent entry into `agents.list[]` → write config → send SIGUSR1. The armada-agent plugin's `/hooks/fleet/agents` endpoint orchestrates this on the instance side.

2. **Credential scoping must use file-based helpers, not env vars.** Since `env.vars` is instance-global, per-agent credentials must be written to workspace-level files (e.g. `.git-credentials`, credential-helper config) rather than injected as environment variables. The existing fleet credential helper already supports this — it writes per-agent credential files into workspace paths.
...
5. **SIGUSR1 restart affects ALL agents in the instance briefly.** When adding or removing an agent, the graceful restart drains all active turns across all agents in that instance (~30s max). Fleet should consider an instance-level drain notification before agent add/remove operations, especially for instances with many active agents.
...
Instance-level logs capture all agent activity. Fleet-control can filter by agent ID in log lines.

```
fleet_logs(target: "dev-team")              → all logs from the instance
fleet_logs(target: "dev-team", agent: "forge") → filtered to forge's lines
```
...
### Scenario A: Initial Fleet Setup
...
1. fleet_transfer("forge", targetNode: "node-2")
   → Fleet creates temp instance on node-2
...
| Term | Definition |
|------|-----------|
| **Armada Control** | Central API server managing the entire fleet |
...
| **Lead Agent** | The primary agent (id: `main`) in an instance — receives fleet tasks |
...
| **Auto-placement** | Fleet selecting the best instance for a new agent |
    ...
    "entries": {
      "armada-agent": {
        ...
        "config": {
          ...
          "contacts": [
            { "name": "nexus", "url": "http://fleet-pm-inst:18789", "role": "project-manager" }
          ]
        }
      }
    }

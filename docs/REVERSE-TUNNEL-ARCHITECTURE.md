...
> RFC — Armada node agents dial out to the control plane instead of being called inbound.
...
- Armada agent plugin hooks (heartbeats, task completion callbacks)
- Nudge responses
...
```
Instance → http://armada-gateway:3002/api/... → Node Agent → WSS → Control Plane
```
...
Instance config only needs: `callbackUrl: "http://armada-gateway:3002"`
...
```
Instance A → armada-gateway → Node A → WSS → Control Plane → WSS → Node B → Instance B
```
...
```
Instance A → armada-gateway → Node Agent → Instance B (local)
```
...
OpenClaw instances use hooks (HTTP callbacks) for armada integration — heartbeats, task completion, nudge responses. Currently these require direct HTTP connectivity between control plane and instances. With the reverse tunnel, **all hook traffic flows through the WebSocket**:
...
- **One endpoint**: `http://armada-gateway:3002` — their gateway to everything
...
docker run -d \
  ...
  -e ARMADA_CONTROL_URL=wss://armada.example.com/api/nodes/ws \
  -e ARMADA_NODE_TOKEN=abc123 \

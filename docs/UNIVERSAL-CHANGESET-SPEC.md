...
> Every fleet mutation flows through the changeset pipeline. The pipeline determines what actions are needed based on what changed.

## Overview

Currently, only agent CRUD, provider, and model mutations go through changesets. This spec extends the pipeline to **all fleet state mutations** and introduces **template sync** — the mechanism that propagates template changes to deployed agents.
...
These are metadata stored in the fleet DB only:
...
> `contacts` are managed by the armada-agent plugin based on mesh/fleet topology — they're not a template concern.

...
export function generateOpenClawConfig(opts: GenerateConfigOptions): { config: object; gatewayToken: string } {
  ...
  // Provision armada-agent plugin if we have orgHooksToken (fleet is enabled)
  if (opts.orgHooksToken) {
    ...
    pluginEntries['armada-agent'] = {
      ...
      config: {
        ...
        fleetApiUrl: process.env.FLEET_CONTROL_URL || process.env.FLEET_AGENT_GATEWAY_URL || 'http://fleet-gateway:3002',
        fleetApiToken: process.env.FLEET_API_TOKEN || '',
        // Node agent proxy URL for remote deployments
        proxyUrl: process.env.FLEET_AGENT_PROXY_URL || '',
      },
    };
  ...
  const config: Record<string, unknown> = {
    ...
    models: {
      ...
      providers: {
        anthropic: {
          apiKey: { source: 'file', provider: 'fleet-creds', id: 'ANTHROPIC_API_KEY' },

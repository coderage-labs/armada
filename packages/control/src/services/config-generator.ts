/**
 * Config Generator — builds openclaw.json config sections from fleet DB state.
 *
 * Translates fleet's model_providers + provider_api_keys + model_registry
...
/**
 * Generate the `models` section of openclaw.json from fleet DB.
 */
...
/**
 * Build the agents.list config section for an instance from fleet DB.
 */
...
export function generateInstanceConfig(instanceId: string): GeneratedConfig {
  ...
  // Build agents list from fleet DB
  ...
  const fleetApiToken = process.env.FLEET_API_TOKEN || '';
  config.plugins = {
    load: {
    ...
    entries: {
      'armada-agent': {
        config: {
          ...
          proxyUrl: process.env.FLEET_AGENT_GATEWAY_URL || 'http://armada-node-agent:3002',
        },
      },
    },
  };

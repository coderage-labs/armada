...
export class ConfigFileLifecycle implements AgentLifecycle {
   ...
   * Inside the instance container, credentials are at /etc/fleet/{agentName}/.
   */
  private async writeAgentGitConfig(instanceName: string, agentId: string): Promise<void> {
    const gitConfigContent = [
      '[credential]',
      `\thelper = /etc/fleet/${agentId}/credential-helper`,
      '',
    ].join('\n');
  ...
  // Config is generated from the fleet DB by config-generator.ts and pushed
  ...
  async createAgent(instanceId: string, config: AgentConfig): Promise<void> {
    ...
    // Note: openclaw.json is NOT written here — it's generated from fleet DB

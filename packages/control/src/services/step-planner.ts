 ...
 * Checks template pinned image → system fleet version → fallback default.
 */
export function resolveInstanceImage(instanceId: string): string {
  ...
  return process.env.FLEET_DEFAULT_IMAGE || 'ghcr.io/openclaw/openclaw:latest';
}
...
export function buildStepsForInstance(instanceId: string, configVersion: number): StepDAG {
  ...
  if (instanceCreateMutation) {
    ...
    const defaultImage = process.env.FLEET_DEFAULT_IMAGE || 'ghcr.io/openclaw/openclaw:latest';
    ...
    nodes[createContainerId] = {
      step: {
        ...
        metadata: {
          ...
          volumes: {
            data: `/data/fleet/instances/${instance?.name ?? instanceId}`,
            plugins: `/data/fleet/instances/${instance?.name ?? instanceId}/plugins`,
          },
          resources: {
          ...
          network: 'armada-net',
          labels: {
            'fleet.instance': instanceId,
            'fleet.instance.name': instance?.name ?? '',
          },
        },
      },
    };
    ...
    nodes[installPluginsId] = {
      step: {
        ...
        metadata: {
          nodeId: resolvedNodeId,
          containerName,
          pluginsDir: `/data/fleet/instances/${instance?.name ?? instanceId}/plugins`,
    ...
    nodes[createContainerId] = {
      step: {
        ...
        metadata: {
          ...
          volumes: instance ? {
            data: `/data/fleet/instances/${instance.name}`,
            plugins: `/data/fleet/instances/${instance.name}/plugins`,
          } : {},
          resources: { memory: resolvedMemory, cpus: resolvedCpus },
          network: 'armada-net',
          labels: instance ? {
            'fleet.instance': instance.id,
            'fleet.instance.name': instance.name,
          } : {},
        },
      },
  ...
  if (needsPluginInstall) {
    ...
    // If no explicit plugins found in mutations, fall back to the fleet agent plugin
    ...
    nodes[installPluginsId] = {
      step: {
        ...
        metadata: {
          nodeId,
          containerName,
          pluginsDir: instance ? `/data/fleet/instances/${instance.name}/plugins` : undefined,

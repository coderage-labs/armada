 ...
 * for sending messages (and nudges) to agents.
 *
...
/**
 * Send a message to a running agent via its `/armada/task` endpoint and
...
export async function sendMessage(
  ...
  const operatorName = opts.callerName || process.env.ARMADA_OPERATOR_NAME || 'system';
...
export async function nudgeAgent(
  ...
  const resolvedCaller = callerName || process.env.ARMADA_OPERATOR_NAME || 'system';
  const controlPlaneUrl = process.env.ARMADA_API_URL || 'http://armada-control:3001';
  const { containerName, nodeId } = resolveAgentRelay(agent);

...
// Use FLEET_API_URL if set, but always use the known armada-control token.
// (The shell may have FLEET_API_TOKEN set to a different server's token — don't inherit it.)
const API_URL = process.env.FLEET_API_URL || 'http://armada-control:3001';
const TOKEN = process.env.FLEET_API_TOKEN || process.env.FLEET_API_TOKEN ?? 'test-token';

...
export function createAgentRoutes(nodeManager: NodeManager): Router {
  ...
  registerToolDef({
    name: 'fleet_status',
    description: 'Check the status of all fleet instances. Shows which agents are running and available for fleet_task.',
  ...
  registerToolDef({
    name: 'fleet_spawn',
    description: 'Spawn a new fleet agent from a template. Contact sync runs automatically after spawn.',
  ...
  registerToolDef({
    name: 'fleet_destroy',
    description: 'Destroy a fleet agent — stops and removes its container, deletes the agent record. Contact sync runs automatically after deletion.',
  ...
  registerToolDef({
    name: 'fleet_logs',
    description: 'Get recent logs from a fleet agent\'s instance container.',
  ...
  registerToolDef({
    name: 'fleet_avatar_generate',
    description: 'Generate an AI avatar image for a fleet agent based on its name and role.',
  ...
  registerToolDef({
    name: 'fleet_avatar_delete',
    description: 'Delete the avatar image for a fleet agent.',
  ...
  registerToolDef({
    name: 'fleet_maintain',
    description: 'Perform graceful maintenance on a fleet agent — drains active tasks, signals graceful restart (SIGUSR1), and waits for the agent to come back healthy. Long-running operation (up to ~2.5 minutes).',
  ...
  registerToolDef({
    name: 'fleet_agent_session_messages',
  ...
  router.get('/stream', (req, res) => {
    const sse = setupSSE(res);
    const unsubscribe = eventBus.on('agent.*', (fleetEvent) => {
      sse.send(fleetEvent.event, fleetEvent.data, fleetEvent.id);
    });
    req.on('close', () => unsubscribe());
  });
  ...
  router.get('/:name/session', async (req, res, next) => {
    try {
      ...
      try {
        const resp = await node.relayRequest(containerName, 'GET', '/fleet/session') as any;
  ...
  router.get('/:name/session/messages', async (req, res, next) => {
    try {
      ...
      const path = `/fleet/session/messages?${qs.toString()}`;

...
const fleetVersion: string = JSON.parse(readFileSync(pkgPath, 'utf-8')).version;
...
registerToolDef({
  name: 'fleet_system_status',
  description: 'Full fleet system status — agent counts, node health, resource usage (CPU, memory, disk).',
...
export function createSystemRoutes(nodeManager: NodeManager): Router {
  ...
  // GET /api/status — fleet summary with resource data
        ...
        try {
          ...
          const containerMap = new Map(
            containers.flatMap((c: any) => {
              const label = c.Labels?.['fleet.agent'] ?? '';
              const state = c.State;
              const stripped = label.startsWith('fleet-') ? label.slice(6) : label;
  ...
  // fleet-wide equivalent of POST /api/plugins/update-all.

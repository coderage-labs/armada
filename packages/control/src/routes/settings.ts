...
registerToolDef({
  name: 'fleet_settings_get',
  description: 'Get all fleet settings (version, retention, avatar generation, sync interval).',
  method: 'GET', path: '/api/settings', parameters: [],
});

registerToolDef({
  name: 'fleet_settings_update',
  description: 'Update a fleet setting by key.',
  method: 'PUT', path: '/api/settings',
  parameters: [
    { name: 'key', type: 'string', description: 'Setting key (fleet_openclaw_version, workspace_retention_days, ai_avatar_generation, github_sync_interval_minutes, avatar_provider_id, avatar_model_id)', required: true },
...
router.get('/', (_req, res) => {
  const fleet_openclaw_version = settingsRepo.get('fleet_openclaw_version') ?? null;

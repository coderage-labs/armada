...
registerToolDef({
  name: 'fleet_inbound_webhooks_list',
...
registerToolDef({
  name: 'fleet_inbound_webhooks_create',
...
registerToolDef({
  name: 'fleet_inbound_webhooks_update',
...
registerToolDef({
  name: 'fleet_inbound_webhooks_delete',
...
receiverRouter.post('/:hookId', async (req, res) => {
  ...
  if (hook.secret) {
    ...
    const sigHeader =
      ...
      (req.headers['x-fleet-signature'] as string | undefined);

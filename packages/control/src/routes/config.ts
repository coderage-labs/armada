...
registerToolDef({
  name: 'fleet_config_status',
  description: 'Get the current fleet config version, stale instances, and pending restarts.',
  method: 'GET', path: '/api/config/status', parameters: [],
});

registerToolDef({
  name: 'fleet_config_snapshot',
  description: 'Take a snapshot of the current fleet config state (providers, models, plugins, template models).',

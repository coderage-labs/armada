/**
 * mdns-scanner.ts — mDNS scanner for fleet node auto-discovery.
 *
 * Periodically queries the local network for `_fleet-node._tcp` services.
...
/**
 * Start scanning for fleet nodes via mDNS.
...
export async function startMdnsScanner(): Promise<void> {
  ...
  console.log('[mdns-scanner] Starting mDNS discovery for fleet nodes');

...
describe('Event Wiring Failures — negative paths', () => {
  ...
  it('multiple event handlers registered, one throws → other handlers still execute (bus isolation)', () => {
    ...
    bus.on('armada.update', () => { executed.push('handler-a'); });
    bus.on('armada.update', () => { throw new Error('handler-b exploded'); });
    bus.on('armada.update', () => { executed.push('handler-c'); });

    // Bus must not propagate the error to the caller
    expect(() => bus.emit('armada.update', { instanceId: 'inst-1' })).not.toThrow();

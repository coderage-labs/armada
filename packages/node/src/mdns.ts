/**
 * mdns.ts — mDNS service advertiser for the armada node agent.
 *
 * Advertises the node as `_armada-node._tcp.local` so that armada control
...
const AGENT_PORT = parseInt(process.env.ARMADA_AGENT_PORT ?? '3002', 10);

/** Node display name — prefer env var, fall back to OS hostname */
function getNodeName(): string {
  return (
    process.env.ARMADA_NODE_NAME ??
...
/** Build the full service instance name: `<nodeName>._armada-node._tcp.local` */

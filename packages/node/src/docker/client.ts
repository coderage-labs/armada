...
export async function createContainer(opts: CreateContainerOptions): Promise<string> {
  ...
  // The node agent's /data maps to HOST_DATA_DIR on the host (default /opt/fleet-data).
  const hostDataDir = process.env.HOST_DATA_DIR || '/opt/fleet-data';
  ...
  // Agent name is derived from labels or container name (strip fleet- prefix)
  const agentName = opts.labels?.['fleet.agent']?.replace(/^fleet-/, '') || opts.name.replace(/^fleet-/, '');
  const credDir = `/data/fleet/${agentName}/credentials`;
  mkdirSync(credDir, { recursive: true });
  const credFile = `${credDir}/git-credentials.json`;
  if (!existsSync(credFile)) {
    writeFileSync(credFile, JSON.stringify({ credentials: [] }), { mode: 0o644 });
  }
  const helperDst = `${credDir}/fleet-credential-helper`;
  ...
  const container = await docker.createContainer({
    ...
    Env: [
      'PATH=/usr/local/fleet-tools:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
      ...(opts.env || []),
      `OPENCLAW_MDNS_HOSTNAME=${opts.name}`,
    ],
    Labels: {
      'fleet.agent': opts.name,
      ...opts.labels,
    },
    ...
    HostConfig: {
      ...
      Binds: [
        ...
        `${hostDataDir}/tools/bin:/usr/local/fleet-tools:ro`,
        // Git credential helper + credentials file
        `${toHostPath(credDir)}:/etc/fleet:ro`,
      ],
  ...
  // Configure git to use the fleet credential helper (as node user, the default runtime user)
  try {
    const gitExec = await container.exec({
      Cmd: ['git', 'config', '--global', 'credential.helper', '/etc/fleet/fleet-credential-helper'],
      User: 'node',
    });
...
/**
 * List all containers managed by the fleet (labelled fleet.agent).
 */
export async function listFleetContainers(): Promise<Docker.ContainerInfo[]> {
  return docker.listContainers({
    all: true,
    filters: { label: ['fleet.agent'] },
  });
}

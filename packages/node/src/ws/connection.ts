...
const CONTROL_URL = process.env.FLEET_CONTROL_URL ?? '';
/** Install token — only used on first connection if no credentials file exists */
const FLEET_NODE_TOKEN = process.env.FLEET_NODE_TOKEN ?? '';
...
export function connectToControlPlane(): void {
  if (!CONTROL_URL) {
    console.warn('[ws] FLEET_CONTROL_URL not set — WebSocket client disabled');
    return;
  }
...
function getAuthToken(): { token: string; nodeId: string | null } {
  ...
  return { token: FLEET_NODE_TOKEN, nodeId: null };
}
...
function startHeartbeat(): void {
  ...
  heartbeatTimer = setInterval(async () => {
    ...
    // Collect fleet container statuses

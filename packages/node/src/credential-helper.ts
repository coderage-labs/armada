/**
 * Git credential helper management for fleet agents.
 *
...
const HELPER_FILENAME = 'fleet-credential-helper';
...
set -e
CRED_FILE="\${CRED_FILE:-/etc/fleet/git-credentials.json}"
 ...
 *
 * This is bind-mounted as /etc/fleet/ inside the container, so each agent's
 * credentials end up at /etc/fleet/{agentName}/git-credentials.json.
 */
...
CRED_FILE="/etc/fleet/${agentName}/git-credentials.json"
export CRED_FILE
exec /etc/fleet/${agentName}/${HELPER_FILENAME} "$@"
`;

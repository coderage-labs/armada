...
const DEFAULT_PLUGINS_DIR = process.env.FLEET_PLUGINS_DIR || '/data/fleet-plugins';

/** Default shared skills library directory */
const DEFAULT_SKILLS_LIBRARY_DIR = process.env.FLEET_SKILLS_DIR || '/data/fleet-library-skills';

/** Base directory where per-instance data volumes live */
const FLEET_INSTANCES_DIR = process.env.FLEET_INSTANCES_DIR || '/data/fleet/instances';
...
async function resolveContainerSkillsDir(containerId: string): Promise<string> {
  ...
  if (!mount?.Source) {
    ...
    // FLEET_INSTANCES_DIR/{name}
    const instanceName = containerId.replace(/^armada-instance-/, '').replace(/^fleet-/, '');
    return join(FLEET_INSTANCES_DIR, instanceName, 'workspace', 'skills');
  }

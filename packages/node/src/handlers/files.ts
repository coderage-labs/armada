...
const SHARED_FILES_DIR = process.env.FLEET_SHARED_FILES_DIR || '/data/fleet/shared-files';
 ...
 * FLEET_INSTANCES_DIR/{name}. As a fallback we also try treating the agent
 * name directly as the instance name.
 */
function agentDataDir(agentName: string): string {
  return `${FLEET_INSTANCES_DIR}/${agentName}`;
}

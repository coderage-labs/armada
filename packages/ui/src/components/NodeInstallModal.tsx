...
const FLEET_ORIGIN =
  (typeof window !== 'undefined' && (window as any).__FLEET_ORIGIN__) ||
...
export default function NodeInstallModal({ nodeId, installToken, onClose }: NodeInstallModalProps) {
  ...
  const curlCommand = `curl -fsSL ${FLEET_ORIGIN}/install | sh -s -- --token ${installToken}`;
  const dockerCommand = `docker run -d --name fleet-node --restart unless-stopped \\
  --memory=256m --memory-swap=512m --cpus=0.5 \\
  -v /var/run/docker.sock:/var/run/docker.sock \\
  -v ~/fleet-data:/data \\
  -v ~/fleet-data/node-credentials:/etc/fleet-node \\
  -e FLEET_CONTROL_URL=wss://${FLEET_ORIGIN.replace(/^https?:\/\//, '')}/api/nodes/ws \\
  -e FLEET_NODE_TOKEN=${installToken} \\
  -e HOST_DATA_DIR=\${HOME}/fleet-data \\

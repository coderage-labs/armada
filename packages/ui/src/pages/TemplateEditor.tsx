...
export default function TemplateEditor() {
  ...
  const [imageMode, setImageMode] = useState<'fleet' | 'custom'>('fleet');
  ...
  useEffect(() => {
    apiFetch<{ fleet_openclaw_version: string | null; latestVersion: string | null }>('/api/settings')
      .then((s) => setFleetVersion(s.fleet_openclaw_version ?? s.latestVersion ?? null))
      .catch(() => { /* ignore */ });
  }, []);
  ...
  useEffect(() => {
    if (!id) return; // new template → default fleet
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const body = {
        ...
        image: imageMode === 'fleet' ? '' : form.image,
        model: form.model,
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as Tab)}>
              ...
              <div className="space-y-2">
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
                    <Input
                      type="radio"
                      checked={imageMode === 'fleet'}
                      onChange={() => setImageMode('fleet')}
                      className="accent-violet-500"
                    />
                    Armada managed{fleetVersion ? ` (v${fleetVersion})` : ''}
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
                    <Input
                      type="radio"
                ...
                {imageMode === 'fleet' && (
                  <p className="text-xs text-zinc-500">
                    Uses the fleet-wide version from{' '}
                    <a href="/settings" className="text-violet-400 hover:underline">Settings</a>.
                    {fleetVersion ? ` Currently: ghcr.io/openclaw/openclaw:${fleetVersion}` : ' Falls back to latest if not configured.'}
                  </p>
                )}
              </div>
            </div>
        ...
        <TabsContent value="Contacts" className="rounded-lg border border-zinc-800 bg-zinc-800/50 p-6 mt-0">
          <div className="space-y-4">
            <p className="text-sm text-zinc-400 mb-2">Define which other fleet instances this agent can communicate with</p>

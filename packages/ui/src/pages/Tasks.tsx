...
function TaskDetailPanel({ task, onClose, onUnblock, onCommentEvent }: { task: ArmadaTask; onClose: () => void; onUnblock?: (id: string) => void; onCommentEvent?: React.MutableRefObject<((data: { taskId: string; comment: TaskComment }) => void) | null> }) {
  ...
  const [commentAuthor, setCommentAuthor] = useState(() => localStorage.getItem('fleet_comment_author') || 'user');
  ...
  async function handleAddComment(e: React.FormEvent) {
    ...
    setSubmitting(true);
    localStorage.setItem('fleet_comment_author', commentAuthor);
    try {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="shrink-0">
        <PageHeader icon={Zap} title="Task Activity" subtitle="Real-time fleet task monitoring & network topology">
          <Button

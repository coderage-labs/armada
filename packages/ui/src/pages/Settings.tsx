...
interface SettingsData {
  fleet_openclaw_version: string | null;
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    setVersionInput(result.fleet_openclaw_version ?? '');
  ...
  const handleSaveVersion = async () => {
    ...
    try {
      await apiFetch('/api/settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: 'fleet_openclaw_version', value: versionInput.trim() || null }),
      });
          ...
          {/* OpenClaw Version */}
            <section className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-4">
              ...
              <p className="text-sm text-zinc-400">
                Set the default OpenClaw image tag used fleet-wide when provisioning instances.
              ...
              {data?.latestVersion && (
                <div className="flex items-center gap-2 text-sm text-green-400 bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2">
                  ...
                  {data.fleet_openclaw_version && data.fleet_openclaw_version !== data.latestVersion && (
                    <span className="ml-1 text-amber-400">(fleet pinned to {data.fleet_openclaw_version})</span>
                  )}
                </div>
              )}
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-4">
              ...
              <p className="text-sm text-zinc-400">
                ...
                Workspaces for stopped agents are preserved for reuse. The fleet checks daily and removes
